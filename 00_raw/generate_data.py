...
Output:
    ...
    data/artists.csv     (~100 records)
    data/tracks.csv      (~1000 records)
...
# For creating casing issues
GENRES_DIRTY = GENRES_CLEAN + [
    "rock", "ROCK", "pop", "POP", "hip hop", "HIP HOP",
    "electronic", "ELECTRONIC", "jazz", "JAZZ"
]

PLATFORMS = ["mobile_ios", "mobile_android", "desktop_app", "web_player", "smart_speaker"]
...
def generate_listeners() -> list[dict[str, Any]]:
    ...
    for i in range(NUM_LISTENERS):
        ...
        # Signup dates spread over 2 years before the streaming year
...
def generate_artists() -> list[dict[str, Any]]:
    ...
    for i in range(NUM_ARTISTS):
        ...
        artist = {
            ...
            "formed_year": random.randint(1960, 2024)
        }
...
def generate_tracks(artists: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for i in range(NUM_TRACKS):
        ...
        # Some tracks have no album (singles)
        has_album = random.random() > 0.2

        # Duration between 90 seconds and 8 minutes
...
def generate_streams(
    ...
    track_durations = {t["track_id"]: t["duration_seconds"] for t in tracks}

    # Premium listeners stream more
    premium_listeners = {l["listener_id"] for l in listeners if l["subscription_tier"] == "premium"}
    ...
    while stream_count < NUM_STREAMS:
        ...
        if random.random() < 0.005:
            ...
            track_duration = 200  # Fake duration for invalid track
        else:
        ...
        # Duration listened (most listen to full song, some skip)
        if random.random() < 0.7:
            # Full listen
            duration_listened = track_duration
        elif random.random() < 0.5:
            # Partial listen (30-90%)
            duration_listened = int(track_duration * random.uniform(0.3, 0.9))
        else:
            # Skip (< 30 seconds)
            duration_listened = random.randint(5, 30)
...
def save_listeners(listeners: list[dict[str, Any]], filepath: str) -> None:
    """Save listeners as JSON."""
    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump(listeners, f, indent=2)
    print(f"Saved {len(listeners)} listeners to {filepath}")
...
def main():
    ...
    # Save files
    save_listeners(listeners, os.path.join(data_dir, "listeners.json"))
    save_csv(artists, os.path.join(data_dir, "artists.csv"))
    save_csv(tracks, os.path.join(data_dir, "tracks.csv"))
    ...
    print("  - ~20% missing album in tracks")
    ...
    print("  - ~2% date format variations")
